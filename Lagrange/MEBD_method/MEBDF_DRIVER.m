function [X_RETURN,Y_RETURN] = MEBDF_DRIVER(TSPAN,y0,options)

%$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$%
%       An extension to MEBDF.m so that it may be used as ODE15s          %
%                                                                         %
%                   Written by O. Selim,                                  %
%                   Student at,                                           %
...
%                   England                                               %
%                                                                         %
%       Please direct any questions or comments(good or bad)to:           %
%                            j.cash@ic.ac.uk                              %
%                                                                         %
% MEBDF_DRIVER calls upon odesetMEBDF.m and odegetMEBDF.m which are       %
% both essentially the original MATLAB functions odeset.m and             %
% odeget.m (respectively), just with minor changes.                        %
%                                                                         %
%                              08/10/2005.                                %
%                                                                         %
%                                                                         %
...
global MYtagSTIFF
MYtagSTIFF = 1;
%CHECK MIN NUMBER OF INPUT ARGUEMENTS.
LOUT = fopen('REPORT.txt','w');

if (nargin<2)
    error('not enough input arguements') 
end

%CHECK THAT TSPAN AND y0 ARE COLUMN VECTORS.
SIZE_TSPAN = size(TSPAN);
SIZE_y0 = size(y0);
if (SIZE_TSPAN(2) == 1)
    disp('TSPAN (1st input arguement) must be a row vector of at least two elements');
    error('size(TSPAN) = %f',SIZE_TSPAN);
elseif (SIZE_y0(2) < SIZE_y0(1))
    disp('y0 (2nd input arguement) must be a row vector');
   error('size(y0) = %f',SIZE_y0);
end

%SET VARIABLE 'INDEXER1' WHICH CONTROLS THE OUTPUT Y.
if (SIZE_TSPAN(2)>2)
    INDEXER1 = 1;
    %THE RETURNED SOLUTION WILL BE AT THOSE SPECIFIED POINTS IN TSPAN.
else
    INDEXER1 = 0;
    %THE RETURNED SOLUTION WILL BE AT THOSE POINTS CHOSEN BY THE SOLVER.
    %RECOMMENDED FOR PLOTS.
end
    

if (nargin<3)
%NOTHING HAS BEEN DECLARED IN options SO SET DEFAULT VALUES AND RUN
%STANDARD SOLVER.    
    
    %SET DEFAULT VALUES. 
    options = 0;
   [N,LWORK,LIWORK,MBND,MASBND,RPAR,IPAR,IERR,IWORK_15,...
    WORK_1,WORK_2,WORK_3,WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,...
    WORK_I6,WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,RTOL,...
    ATOL,H,ITOL,MF,INDEX,MAXDER,IWORK,Y_RETURN,Y ,...
    X_RETURN]= initial_value_set(0,0,SIZE_y0,TSPAN,options,y0);

    
    %CHOOSE OUTPUTS WITH INDEXER1.
    [N,X,H,Y,XOUT,XEND,MF,INDEX,LOUT,LWORK,WORK_1,WORK_2,WORK_3,...
                    WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,WORK_I6,...
                    WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,LIWORK,...
                    IWORK,IWORK_15,MBND,MASBND,MAXDER,ITOL,RTOL,ATOL,...
                    RPAR,IPAR,IERR,Y_RETURN,X_RETURN,TIME]= ...
                    output_selecter(INDEXER1,TSPAN,...
                    Y_RETURN,X_RETURN,N,H,Y,MF,INDEX,...
                    LOUT,LWORK,WORK_1,WORK_2,WORK_3,WORK_I1,WORK_I2,...
                    WORK_I3,WORK_I4,WORK_I5,WORK_I6,WORK_I7,WORK_I8,...
                    WORK_I9,WORK_I10,WORK_I11,LIWORK,IWORK,IWORK_15,...
                    MBND,MASBND,MAXDER,ITOL,RTOL,ATOL,RPAR,IPAR,IERR);
    
elseif (nargin == 3)
%options HAS BEEN DECLARED SO SET VALUES ACCORDINGLY.
    
    BNDED =  odegetMEBDF(options,'banded','off');
    
    if strcmp(BNDED,'on') 
        %SET VALUES OF MASBND AND MBND IN THE APPROPRIATE PLACES.  
        
      [N,LWORK,LIWORK,MBND,MASBND,RPAR,IPAR,IERR,IWORK_15,...
    WORK_1,WORK_2,WORK_3,WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,...
    WORK_I6,WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,RTOL,...
    ATOL,H,ITOL,MF,INDEX,MAXDER,IWORK,Y_RETURN,Y ,...
    X_RETURN]= initial_value_set(1,0,SIZE_y0,TSPAN,options,y0);
    ...
    elseif strcmp(BNDED,'off')     
        
       [N,LWORK,LIWORK,MBND,MASBND,RPAR,IPAR,IERR,IWORK_15,...
    WORK_1,WORK_2,WORK_3,WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,...
    WORK_I6,WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,RTOL,...
    ATOL,H,ITOL,MF,INDEX,MAXDER,IWORK,Y_RETURN,Y ,...
    X_RETURN]= initial_value_set(1,1,SIZE_y0,TSPAN,options,y0);
...
stat_on_or_off = odegetMEBDF(options,'stats','off');

if strcmp(stat_on_or_off,'on')
    ...
    fprintf(LOUT,'\r\r COMPUTATIONAL STATISTICS OF RUN');
    fprintf(LOUT,'\r\r NUMBER OF EQUATIONS: %g',N);
    fprintf(LOUT,'\r\r RESULTS WITH THE FOLLOWING TOL :');
    fprintf(LOUT,'\r RTOL = %E',RTOL);
    fprintf(LOUT,'\r ATOL = %E',ATOL);
    fprintf(LOUT,'\r\r\r LAST STEP SIZE: %E',HUSED);
    fprintf(LOUT,'\r LAST ORDER OF THE METHOD: %g',NQUSED);
    fprintf(LOUT,'\r MAXIMUM ORDER USED SO FAR: %g',MAXORD);
    fprintf(LOUT,'\r TOTAL NUMBER OF STEPS TAKEN: %g',NSTEP);
    fprintf(LOUT,'\r TOTAL NUMBER OF FAILED STEPS: %g',NFAIL);
    fprintf(LOUT,'\r NUMBER OF FUNCTION EVALUATIONS: %g',NFE);
    fprintf(LOUT,'\r NUMBER OF JACOBIAN EVALUATIONS: %g',NJE);
    fprintf(LOUT,'\r NUMBER OF FACTORIZATION: %g',NDEC);
    fprintf(LOUT,'\r NUMBER OF BACKSOLVES: %g',NBSOL);
    fprintf(LOUT,'\r CPU TIME: %g',TIME);
...
ploter1 =odegetMEBDF(options,'one_plot','off');
ploter2 =odegetMEBDF(options,'sep_plots','off');
ploter3 =odegetMEBDF(options,'y_plots','off');

if ~(strcmp(ploter1,'off'))
    figure
    for i = 1:N

        subplot(ploter1(1),ploter1(2),i)
        plot(X_RETURN,Y_RETURN(:,i));
        xlabel('X');
        [LAB, errmsg] = sprintf('Y%g',i);
        ylabel(LAB);
    end
    
elseif strcmp(ploter2,'on')
     for i = 1:N
        figure
        plot(X_RETURN,Y_RETURN(:,i));
        xlabel('X');
...
elseif strcmp(ploter3,'on')

   if (N~=2)
       disp('error using ''y_plots'', there are more then two equations')
   else
       figure
       plot(Y_RETURN(:,1),Y_RETURN(:,2))
       xlabel('y1');
...
%**************************************************************************
%**************************************************************************
function  [N,X,H,Y,XOUT,XEND,MF,INDEX,LOUT,LWORK,WORK_1,WORK_2,WORK_3,...
                    WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,WORK_I6,...
                    WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,LIWORK,...
                    IWORK,IWORK_15,MBND,MASBND,MAXDER,ITOL,RTOL,ATOL,...
                    RPAR,IPAR,IERR,Y_RETURN,X_RETURN,TIME]= ...
    ...
    if INDEXER1 == 0
        ...
        while (MYtag == 0)
                ...
                case 220
                    ...
                    INDEX = 3;
                    SIZE_Y_RETURN = size(Y_RETURN);
                    Y_RETURN(SIZE_Y_RETURN(1)+1,:) = Y;
                    SIZE_X_RETURN = size(X_RETURN);
                    X_RETURN(SIZE_X_RETURN(2)+1) = X;
        ...
        for I = 1:(SIZE_TSPAN(2)-1)
            ...
            while (MYtag == 0)
                switch MYcase
                    ...
                    Y_RETURN(SIZE_Y_RETURN(1)+1,:) = Y;
                    SIZE_X_RETURN = size(X_RETURN);
                    X_RETURN(SIZE_X_RETURN(2)+1) = XOUT;
...
%**************************************************************************
%**************************************************************************
function [N,LWORK,LIWORK,MBND,MASBND,RPAR,IPAR,IERR,IWORK_15,...
    WORK_1,WORK_2,WORK_3,WORK_I1,WORK_I2,WORK_I3,WORK_I4,WORK_I5,...
    WORK_I6,WORK_I7,WORK_I8,WORK_I9,WORK_I10,WORK_I11,RTOL,...
    ATOL,H,ITOL,MF,INDEX,MAXDER,IWORK,Y_RETURN,Y ,...
    X_RETURN]= initial_value_set(type1,type2,SIZE_y0,TSPAN,options,y0)

%  type1 = {0,1}
%  type2 = {0,1}
% 'type1' determines wether or not to use 'options' to set values.
% 'type2' determines wether or not to use BANDED set up.

if (type1 == 0) %set default values.
...
% C     <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    if (type2 == 0)
     MYtagSTIFF = 1;
        ...
        MYY = zeros(1,20);
        MYVAR = 0;
        MBND   = odegetMEBDF(options,'mbnd',zeros(1,4));
        MASBND = odegetMEBDF(options,'masbnd',zeros(1,4));
        RPAR = 0;
        IPAR = 0;
        IERR = 0;
        N=ND;
...
% C     <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        RTOL=odegetMEBDF(options,'rtol',1E-3);
        ATOL=odegetMEBDF(options,'atol',1E-6);
        ITOL=odegetMEBDF(options,'itol',2);
        H = odegetMEBDF(options,'hstart',1E-6);
        MF=odegetMEBDF(options,'mf',21);
        INDEX=1;
        MAXDER=odegetMEBDF(options,'maxder',7);
        IWORK(14)=odegetMEBDF(options,'I_WORK_14',200000);
        NART = 20; 
        Y_RETURN(1,:) = y0;
        Y = y0;
        X_RETURN = TSPAN(1);
        
        IWORK(1) = odegetMEBDF(options,'Iwork_1',0);
        IWORK(2) = odegetMEBDF(options,'Iwork_2',0);
        IWORK(3) = odegetMEBDF(options,'Iwork_3',0);
    else
         MYtagSTIFF = 1;
        ND = SIZE_y0(2);
        LWORK =(33+3*ND)*ND+3;
...
% C     <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        INDEX=1;
        MAXDER=odegetMEBDF(options,'maxder',7);
        IWORK(14)=odegetMEBDF(options,'I_WORK_14',2000000);
        %MASBND(1)=0;
        Y_RETURN(1,:) = y0;
        Y = y0;
        X_RETURN = TSPAN(1);
...
function o = odegetMEBDF(options,name,default)
...
function [N, T0, HO, Y0, TOUT, TEND, MF, IDID, LOUT, LWORK, WORK_1, ...
    ...
    IPAR, IERR] = MEBDF(N, T0, HO, Y0, TOUT, TEND, MF, IDID, LOUT, ...
...
if (IDID == 1)
    ...
    if (N <= 0)
        ...
        fprintf(LOUT,['*** ERROR ***** INTEGRATION HALTED IN DRIVER \n >>>' ...
...
[N, T0, HO, Y0, TOUT, TEND, MF, IDID, LOUT, WORK_3, WORK_I1, WORK_I2, ...
    ...
    EPSJAC, IERR] = OVDRIV(N, T0, HO, Y0, TOUT, TEND, MF, IDID, LOUT, WORK_3, ...
...
function [N, T0, HO, Y0, TOUT, TEND, MF, IDID, LOUT, Y, YHOLD, YNHOLD, ...
...
elseif (IDID==3)
    ...
    if ((ITOL==1) && (RTOL(1)== 0))
        ...
        fprintf(LOUT,' ILLEGAL INPUT.. RTOL .LE. 0. \n');
        %WRITE(LOUT,9040)
        IDID = -4;
    end
    ...
    if (N <= 0)
        ...
        fprintf(LOUT,' ILLEGAL INPUT.. N .LE. 0 \n');
        %WRITE (LOUT,9050)
        IDID = -4;
    ...
    switch MYcase
        ...
        case 20 %**********************************************************
            ...
            [H, HMAX, HMIN, JSTART, KFLAG, MF, MBND, MASBND, NIND1, NIND2, NIND3, ...
                ...
                AM, YHOLD, YNHOLD, ARH, IPIV, LOUT, MAXDER, ITOL, RTOL, ATOL, RPAR, ...
            ...
            elseif (KGO == 4)
                ...
                fprintf(LOUT,[' \r\rKFLAG = -3 FROM INTEGRATOR AT T = %f \n  CORRECTOR CONVERGENCE COULD NOT BE ACHIEVED \n',T]);
                %WRITE (LOUT,9030)
                %GOTO 60
                MYcase = 60;
                continue
...
function [H, HMAX, HMIN, JSTART, KFLAG, MF, MBND, MASBND, NIND1, NIND2, ...
    ...
    PW, PWCOPY, AM, YHOLD, YNHOLD, ARH, IPIV, LOUT, MAXDER, ITOL, ...
    ...
    YNHOLD, ARH, IPIV, LOUT, MAXDER, ITOL, RTOL, ATOL, ...
...
while (MYstop == 0)
    ...
    switch MYcase
        ...
        case 8000
            ...
            if (IERR~=0)
                fprintf(LOUT,['IERR IS NON-ZERO BECAUSE OF AN ILLEGAL' ...
                    ' FUNCTION CALL']);
                %write(LOUT,1975)             
                h= h/2;
                if (H<EPSJAC/100)
                    fprintf() %WRITE(6,9161)
